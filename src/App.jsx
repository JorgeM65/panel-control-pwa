...
.header-titles { display: flex; flex-direction: column; cursor: pointer; }
.eyebrow {
  font-family: var(--font-mono);
  font-size: 11px;
...
function TareasTab({ tasks, onChange, onDelete, fabSignal }) {
  ...
  useEffect(() => {
    if (fabSignal) inputRef.current?.focus();
...
function CalendarioTab({ events, onChange, onDelete, fabSignal }) {
  ...
  useEffect(() => {
    if (fabSignal) openAdd(dateKey(new Date()));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fabSignal]);
...
function AjustesTab({ entertainment, onChangeEntertainment }) {
...
function HabitosTab({ habits, onChange, onDelete, fabSignal }) {
  const [text, setText] = useState('');
  const [editingId, setEditingId] = useState(null);
...
function Home({ tasks, events, habits, compra, capsulas, footballMatches, footballConfig, onNavigate, onCompleteTask, onToggleHabit }) {
  const todayKey = dateKey(new Date());
  const order = { alta: 0, media: 1, baja: 2 };
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [t, e, ent, h, c, cap, n] = await Promise.all([
        load('tareas', []),
        load('calendario', []),
  ...
  return (
    <div className="app-shell">
      ...
      <main className="app-card">
        ...
        {activeView === 'ajustes' && <AjustesTab entertainment={entertainment} onChangeEntertainment={updateEntertainment} />}
      </main>
